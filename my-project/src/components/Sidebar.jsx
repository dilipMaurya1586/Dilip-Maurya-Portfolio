...
const Sidebar = ({ isOpen, toggleSidebar, selectedColor, isDarkMode, showTopControls = true }) => {
  ...
  // Close sidebar automatically when a link is tapped (mobile UX)
  ...
  return (
    <>
      ...
      <button
        onClick={toggleSidebar}
        className={`fixed top-4 left-4 z-50 md:hidden p-2 rounded-md transition-all duration-300 ${
          (showTopControls || isOpen) ? 'translate-y-0 opacity-100' : '-translate-y-20 opacity-0 pointer-events-none'
        }`}
